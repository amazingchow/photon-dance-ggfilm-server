...
def render_nginx_custom_conf(root):
    ...
    ggfilm_http_server_template = lookup.get_template("/ggfilm-http-server.conf.example")
    values = {
        "project_path": root,
    }
    content = ggfilm_http_server_template.render(**values)
    content = str(content, encoding = "utf8")
    with open("{}/nginx/ggfilm-http-server.conf".format(root), "w") as fw:
        fw.write(content)

    ggfilm_https_server_template = lookup.get_template("/ggfilm-https-server.conf.example")
    values = {
        "project_path": root,
    }
    content = ggfilm_https_server_template.render(**values)
    content = str(content, encoding = "utf8")
...
def render_uwsgi_custom_ini(root):
    ...
    ggfilm_template = lookup.get_template("/uwsgi.ini.example")
    values = {
        "project_path": root,
    }
    content = ggfilm_template.render(**values)
    content = str(content, encoding = "utf8")
